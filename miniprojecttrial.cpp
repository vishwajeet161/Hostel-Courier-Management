...
public:
    ...
    {
        srand(time(0));
        int x = rand() % 9000 + 1000;        // range 1000 - 9999 
        OTP = to_string(x);
    }
    ...
    bool send_notification()
    {
        string email_id;
        fstream f;
        if (Address_of_Delivery.getHostel() == "ChandraSekharAzad")
        {
            int flag = 0;
            string word, filename;
            filename = "ChandraSekharAzad.txt";
        ...
        else if (Address_of_Delivery.getHostel() == "SCBose")
        {
            int flag = 0;
            string word, filename;
            filename = "SCBose.txt";
        ...
        {
            fstream fio;
            fio.open(email_id + ".txt", ios::out); //overwrites the data inside the file.
            string message = "There is a parcel for you with Parcel Id " + Parcel_Id + ". Use " + OTP + " as OTP during checkout. \n This is automatically genrated e-mail, Don't reply to it.";
            // \n\t\t This Mail has been successfully Sent to " + email_id + ".\n";
            fio << "From:"
                << "geu Courier Management System"
                << "<geucouriermanagement@gmail.com>" << endl
                << "To:"
            ...
            fio.close();
            // string str =
            //     "curl -s --url 'smtps://smtp.gmail.com:465' --ssl-reqd   --mail-from 'geucouriermanagementsystem@gmail.com' --mail-rcpt '" + email_id +
            //     "'   --upload-file message.txt --user 'geucouriermanagementsystem@gmail.com:vvy616263' --insecure";
    ...
    void getaddress()
    {
        cout << "Hostel-: " << Address_of_Delivery.getHostel() << " Room-no--: " << Address_of_Delivery.getRoom_no() << endl;
    }
};
    ...
    while (i--)
    {
        ...
        if (Del_Man.verify_id() == true)
        {
            ...
            do
            {
                ...
                cout << "Enter Room No :-\n";
                cin >> room;
                if (hostel == "ChandraSekharAzad" || hostel == "SCBose")
                {
                    x = 1;
                }
                else
    ...
    if (itr != map_ID_OTP_Email_id.end())
    {
        ...
        while (tryit > 0)
        {
            ...
            if (itr->second.first == totp)
            {
                ...
                string email_id = itr->second.second;
                fstream fio;
                fio.open(email_id + ".txt", ios::out);
                string message = "Your Parcel with Id " + itr->first + " , has been successfully Checkout. \n This is automatically genrated e-mail, Don't reply to it. ";
                fio << "From:"
                    << "Graphic Era Deemed University Courier Management System"
                    << "<graphicerauniversitycouriermanagement@gmail.com>" << endl
                    << "To:"
                ...
                fio.close();
                // string str =
                //     "curl -s --url 'smtps://smtp.gmail.com:465' --ssl-reqd   --mail-from 'geucouriermanagementsystem@gmail.com' --mail-rcpt '" + itr->second.second +
                //     "'   --upload-file message.txt --user 'geucouriermanagementsystem@gmail.com:vvy616263' --insecure";
...
int main()
{
    int c;
    /* Menu Driven Program */
    cout << "\t\t\tWelcome to Graphic Era Deemed Universty Courier Management System\n";
    while (1)
    {
